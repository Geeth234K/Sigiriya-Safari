...
export default function AboutSection({ data }) {
  return (
    <section className="relative bg-[#050b08] py-20 text-white">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(16,90,60,0.35),_transparent_55%)]" />
      <div className="relative mx-auto grid max-w-6xl gap-12 px-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {data.highlights.map((item) => (
              ...
              >
                ...
                <p className="mt-2 text-sm leading-relaxed text-emerald-100/75">{item.text}</p>
              </div>
            ))}
          </div>
        </div>
        <div className="relative">
          ...
          <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-white/5 shadow-2xl shadow-black/40 backdrop-blur">
            ...
            <div className="absolute bottom-6 left-6 right-6 rounded-2xl border border-white/10 bg-white/10 p-4 text-sm text-emerald-50 backdrop-blur">
              {data.caption}
            </div>
          </div>
        </div>
